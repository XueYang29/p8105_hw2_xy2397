...
``` r
# read the Mr. Trash Wheel sheet from excel file through a relative path
# specify the sheet and omit columns containing notes
# clean up variable names
...
From the Mr. Trash Wheel sheet, there are 285 observations of dumpsters. The dataset contains 14 variables, for example of key variables like dumpster (the number of dumpster), data (year/month/day), weight(tons), volume (cubic yards), different types of trash (plastic bottles, polystyrene, cigarette butts and so on) and homes powered.

From the 2016 and 2017 precipitation sheets, each of them has 12 observations from 12 months. The datasets contains variables including month and monthly total precipitation.
